...
.controller('rootCtrl', ['$scope', '$rootScope', '$uibModal', 'User', '$timeout', '$http', 'ngProgressFactory', '$window', '$localStorage',  function($scope, $rootScope, $uibModal, User, $timeout, $http, ngProgressFactory, $window, $localStorage) {
    ...
    $rootScope.apiURL = window.location.protocol + "//" + window.location.host + "/";
    ...
    $rootScope.loggedIn = $rootScope.$storage.loggedIn || false;
    console.log('loggedIn: ');
    console.log($rootScope.$storage.loggedIn);
    var isMobile = false;
    ...
    $scope.logout = function () {
        delete $rootScope.$storage.loggedIn;
    ...
    $scope.selectCate = function(){
        ...
        var modalInstance = $uibModal.open({
            size: "lg",
            windowTemplateUrl: "/templates/post/window.html",
            templateUrl: "/templates/post/select_categories.html",
            controller: "ComposeCtrl"
...
.controller('ComposeCtrl', ['$scope','$http','$location','$rootScope', '$uibModal', '$uibModalInstance', '$state', function($scope,$http,$location,$rootScope, $uibModal, $uibModalInstance, $state) {
    ...
    $scope.currentTab = 'text.tpl.html';
    ...
    $scope.save = function(){
        ...
        var params = {
            ...
            user_id: 1, //set test value
            type: $scope.currentTab.split('.')[0]
        ...
        $http.post('/api/post/createpost', params )
            .success(function(data, status, headers, config) {
                if(data.error == 0){
...
.controller('LoginCtrl', ['$scope','$http','$location','headerFooterData','$uibModalInstance','$uibModal','AuthService', '$rootScope', '$localStorage', function($scope, $http, $location, headerFooterData, $uibModalInstance, $uibModal, AuthService, $rootScope, $localStorage) {
    ...
    $scope.loginFacebook = function () {
        FB.login(function(response) {
            if (response.authResponse) {
                FB.api('/me', function(response) {
                    ...
                    $scope.state = "success";
                    $rootScope.loggedIn = $rootScope.$storage.loggedIn;
                    $rootScope.$broadcast("login-success", {});
                    $uibModalInstance.close("success");
        ...
        }, {scope: 'user_birthday,user_friends,email,public_profile,user_likes,user_posts,publish_actions', return_scopes: true});
    };
    $scope.closeModal = function () {
        $uibModalInstance.close()
    };
...
.controller('HomeCtrl', ['$scope','$http','$location', 'Post', '$timeout', '$rootScope', 'ngProgressFactory', function($scope,$http,$location, Post, $timeout, $rootScope, ngProgressFactory) {
    ...
    $scope.nextPage = function() {
        ...
        $http.get('/api/post/getlistposts/'+$scope.offset+'/'+$scope.numPerPage)
            .success(function(data, status, headers, config) {
                if(data.error == 0){
                    var items = data.data;
